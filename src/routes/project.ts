...
function getTextField(form: FormData, key: string): string | null {
  const v = form.get(key);
  if (typeof v === "string") return v;
  return null;
}

async function parseImageFile(form: FormData) {
  ...
  if (!image || !(image instanceof Blob) || !isFileLike) return null;

  const arrayBuffer = await (image as Blob).arrayBuffer();
  const originalName = (image as any).name as string;
...
projectRoute.post("/", authMiddleware, async (c) => {
  ...
  if (!title || !description || !techStack) {
    return c.json(
      { message: "title, description, techStack wajib diisi" },
      400
    );
  }
  ...
  if (imageUpload) {
    const saved = await saveProjectImageFromBuffer(
      imageUpload.buffer,
